...
public class WordFormTemplate implements Serializable {
	...
	public void build(SimpleWord w) {
...
//				}
				if (pl != null) {
					...
					if (replace != null && replace.length() > 0
						...
						for (String s: rr){
						wordNet.registerWordForm(s, getRelation((SimpleWordNet) wordNet, w,f));
						}
						continue;
					}
				}
	...
	private WordRelation getRelation(SimpleWordNet wn, SimpleWord w, Field f) {
		Class<?> declaringClass = f.getDeclaringClass();
		HashMap<Field, Integer> hashMap = mm.get(declaringClass);
		Integer integer = hashMap.get(f);
		if (integer!=null){
			return new WordRelation(wn,w, integer);
		}
		return new WordRelation(wn,w, 1012);
	}
